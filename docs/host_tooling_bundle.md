...
| Bruno GUI | API 调试与接口集合管理 | 已安装并可用 | `winget list --id Bruno.Bruno`；GUI：`C:\Users\Donki\AppData\Local\Microsoft\WinGet\Packages\Bruno.Bruno_Microsoft.Winget.Source_8wekyb3d8bbwe\Bruno.exe` | 本地接口调试、接口集合维护 |
...
| Fiddler Everywhere | GUI 抓包与会话分析 | 已安装并可用 | `winget list --id Telerik.Fiddler.Everywhere`；GUI：`C:\Users\Donki\AppData\Local\Programs\Fiddler Everywhere\Fiddler Everywhere.exe` | 图形化查看会话、调试代理流量 |
| FlaUInspect | Windows UIA 树检查工具（FlaUI 辅助） | 已安装并可用 | `winget list --id FlaUI.FlaUInspect`；GUI：`C:\Users\Donki\AppData\Local\Microsoft\WinGet\Packages\FlaUI.FlaUInspect_Microsoft.Winget.Source_8wekyb3d8bbwe\FlaUInspect.exe` | 桌面自动化前的控件树探查与定位 |
...
- 验证结果：`bru 3.2.1`；`winget list --id Bruno.Bruno` 显示 `3.2.0`。
...
- WinAppDriver 最小验证：`C:\Program Files (x86)\Windows Application Driver\WinAppDriver.exe /?` 可正常输出帮助；短暂启动 2 秒后进程仍存活，随后主动结束，说明程序可正常拉起。

## PATH 刷新说明

- 本次安装后，当前 agent 所在 PowerShell 会话未自动刷新新 PATH。
- 因此验证时优先使用绝对路径或 `winget list`。
- 对用户后续新开终端，通常可直接使用以下命令；若仍不可用，可重新打开终端后再试：`gh`、`bru`、`trivy`、`syft`、`FlaUInspect`。
...
| Bruno GUI | `winget list --id Bruno.Bruno` 命中 `3.2.0` |
| Fiddler Everywhere | `winget list --id Telerik.Fiddler.Everywhere` 命中 `7.7.2` |
| FlaUInspect | `winget list --id FlaUI.FlaUInspect` 命中 `3.0.0` |
| WinAppDriver | `WinAppDriver.exe /?` 正常输出帮助；短暂启动可拉起 |

## 结论

- 无迁移，直接补装。
- 本次目标清单已全部落地到可用状态，且已记录验证路径。
