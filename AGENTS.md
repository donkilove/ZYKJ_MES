...
- 编码前必须完成 Sequential-Thinking 分析；若 `Sequential Thinking` 不可用，允许改以当前可用工具链或显式书面推演完成等效拆解，并在任务日志记录降级原因、时间与补偿措施，执行时保持最小变更边界。
- 工作流管理统一使用 `update_plan` 或 `TodoWrite` 维护在制项；若计划工具不可用，允许在任务日志中维护步骤、状态、验收标准与更新时间，不得因此中断等待。
...
- 默认测试基线为：后端使用 `pytest`；前端使用 `flutter test` 与 `integration_test`。
- `desktop_tests/flaui/` 仅作为历史保留/按需 fallback 的桌面系统级自动化例外规则；仅在历史回放或用户显式要求桌面系统级场景时使用，且仍须串行执行，禁止并发多进程或多桌面会话并行跑同一批 FlaUI 用例。
- 用户模块后续新增、补齐或回补边角分支时，必须按同一批次同步收敛后端/API、Flutter、`integration_test`、`evidence/` 与执行命令口径；不得仅改其中一端后留待后补。
- 用户明确要求采用“指挥官模式”或仓库存在 `指挥官工作流程.md` 时，主 agent 必须默认按指挥官流程执行，除非用户显式取消该模式。
...
### 4.6 技能使用
- 仓库新增或更新技能（skills）后，允许按任务需要主动加载并使用，无需额外征求用户许可。
- 选择技能时应优先匹配任务目标、改动范围与验证链路；若技能与当前任务不匹配，可直接使用常规工具链执行。

## 5. 标准工作流
### 5.1 最小循环
1. Research：使用 Serena/Context7/Sequential Thinking 拆解问题，记录约束与假设。
2. Plan：通过 `update_plan` 或 `TodoWrite` 维护步骤、状态与验收标准。
3. Implement：默认按原子任务派发执行子 agent，小步提交，保持最小变更并补充中文文档/注释。
4. Verify：默认派发独立验证子 agent，运行必要的构建、测试、性能与回归检查；未形成独立验证结论不得视为完成。
5. Deliver：总结变更、风险、验证结果，并在有外呼时附“工具调用简报”。

### 5.1.1 指挥官模式附加门禁
1. 先拆解后实现：未完成原子任务拆分与验收定义前，不得直接进入业务改动。
2. 先留痕后交付：未在 `evidence/` 形成任务日志或未更新既有日志前，不得宣称任务已按指挥官流程执行。
3. 先验证后过关：未经过独立验证子 agent 的真实验证，不得把任务标记为“已完成”。
4. 失败必重派：验证未通过时，不得以“基本可用”或“MVP”收口，必须继续闭环。
5. 硬阻塞可收口：仅当客观条件导致无法继续时，才允许记录阻塞、继续完成其余可推进项，并在最终交付统一说明影响与建议。

### 5.2 阶段关卡
阶段关卡仅用于内部质控、任务编排与证据组织，不构成面向用户触发分阶段进度汇报、部分交付或中途收口的条件。
