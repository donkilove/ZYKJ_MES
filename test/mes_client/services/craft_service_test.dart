...
void main() {
  group('CraftService', () {
    test(
      'covers stage/process/template and system-master operations',
      () async {
        final server = await TestHttpServer.start({
          ...
          'PUT /craft/stages/1': (request) {
            ...
            return TestResponse.json(
              200,
              body: {'data': _stageJson(isEnabled: false)},
            );
          },
          'DELETE /craft/stages/1': (_) =>
          ...
          'PUT /craft/processes/2': (request) {
            ...
            return TestResponse.json(
              200,
              body: {'data': _processJson(isEnabled: false)},
            );
          },
          'DELETE /craft/processes/2': (_) =>
              TestResponse.json(200, body: {'data': {}}),
          ...
          'GET /craft/kanban/process-metrics': (request) {
            expect(request.uri.queryParameters['product_id'], '10');
                  ...
                  'items': [
                    {
                      ...
                      'samples': [
                        {
                          'order_process_id': 1000,
                          'order_id': 2000,
                          'order_code': 'PO-001',
                          'start_at': '2026-03-01T00:00:00Z',
                          'end_at': '2026-03-01T00:30:00Z',
                          'work_minutes': 30,
                          'production_qty': 60,
                          'capacity_per_hour': 120.0,
                        },
                      ],
                    },
                  ],
        ...
        final metrics = await service.getCraftKanbanProcessMetrics(
          productId: 10,
          limit: 5,
        );
    ...
    test(
      'throws ApiException and supports non-json response parsing',
      () async {
        final server = await TestHttpServer.start({
          'DELETE /craft/templates/3': (_) =>
              const TestResponse(statusCode: 500, body: 'plain text error'),
        });
        addTearDown(server.close);
