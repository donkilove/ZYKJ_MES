...
DEFAULT_SCAN_PATHS = ("mes_client/lib", "mes_client/test", "test/mes_client")
...
def scan_file(path: Path) -> list[Finding]:
    ...
    for line_no, line_text in enumerate(text.splitlines(), start=1):
        replacement_index = line_text.find("\ufffd")
        if replacement_index >= 0:
            findings.append(
                Finding(
                    path=path,
                    line=line_no,
                    column=replacement_index + 1,
...
def scan_paths(paths: list[str]) -> list[Finding]:
    findings: list[Finding] = []
    for file_path in _iter_scan_files(paths):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Scan frontend Dart source for Chinese mojibake text."
    )
    parser.add_argument(
        "--path",
        action="append",
    ...
    parser.add_argument(
        "--fix",
        action="store_true",
        help="Auto-repair GBK/UTF-8 mojibake and re-scan.",
    )
    ...
    if findings:
        ...
        print(f"Detected potential frontend mojibake issues: {len(findings)}")
        return 1

    print("No frontend Chinese mojibake detected.")
    return 0
