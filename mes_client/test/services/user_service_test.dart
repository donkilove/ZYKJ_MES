...
void main() {
  group('UserService', () {
    test('covers list/create/update/delete and registration review APIs', () async {
          ...
          return TestResponse.json(
            ...
            body: {
              'data': {
                'total': 1,
                'items': [
                  {'id': 10, 'code': 'system_admin', 'name': '系统管理员'},
                ],
              },
            },
          );
      ...
      final users = await service.listUsers(page: 2, pageSize: 20, keyword: '  tester ');
    ...
    test('throws ApiException on non-200 response', () async {
      final server = await TestHttpServer.start({
        'GET /roles': (_) => TestResponse.json(500, body: {'message': 'roles failed'}),
      });
      addTearDown(server.close);
